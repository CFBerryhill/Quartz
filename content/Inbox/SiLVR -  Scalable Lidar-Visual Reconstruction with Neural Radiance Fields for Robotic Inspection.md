---
Title: "SiLVR: Scalable Lidar-Visual Reconstruction with Neural Radiance Fields for Robotic Inspection"
Authors: Yifu Tao; Yash Bhalgat; Lanke Frank Tarimo Fu; Matias Mattamala; Nived Chebrolu; Maurice Fallon
Abstract: "We present a neural-field-based large-scale reconstruction system that fuses lidar and vision data to generate high-quality reconstructions that are geometrically accurate and capture photo-realistic textures. This system adapts the state-of-the-art neural radiance field (NeRF) representation to also incorporate lidar data which adds strong geometric constraints on the depth and surface normals. We exploit the trajectory from a real-time lidar SLAM system to bootstrap a Structure-from-Motion (SfM) procedure to both significantly reduce the computation time and to provide metric scale which is crucial for lidar depth loss. We use submapping to scale the system to large-scale environments captured over long trajectories. We demonstrate the reconstruction system with data from a multi-camera, lidar sensor suite onboard a legged robot, hand-held while scanning building scenes for 600 metres, and onboard an aerial robot surveying a multi-storey mock disaster site-building. Website: https://ori-drs.github.io/projects/silvr/"
Date Published: 2024-05-01
Date found: 2025-06-16
Keywords: Radiance Field, Neural Radiance Fields, Computation Time, Geometric Constraints, Lidar Data, Large-scale Environments, Metric Scale, Surface Normals, Neural Field, Reconstruction System, Legged Robots, Large-scale Reconstruction, 3D Reconstruction, Point Cloud, Multilayer Perceptron, Camera Images, Real-world Datasets, Peak Signal-to-noise Ratio, Multiple Platforms, Reconstruction Quality, View Synthesis, Camera Pose, Lidar Measurements, Complete Reconstruction, LiDAR Point Clouds, Single Map, Bundle Adjustment, Dense Reconstruction, Uniform Texture, Robotic Platform
Link: https://ieeexplore.ieee.org/abstract/document/10611278
DOI: 10.1109/ICRA57147.2024.10611278
Read status: Abstract
---
# Data
## Abstract
We present a neural-field-based large-scale reconstruction system that fuses lidar and vision data to generate high-quality reconstructions that are geometrically accurate and capture photo-realistic textures. This system adapts the state-of-the-art neural radiance field (NeRF) representation to also incorporate lidar data which adds strong geometric constraints on the depth and surface normals. We exploit the trajectory from a real-time lidar SLAM system to bootstrap a Structure-from-Motion (SfM) procedure to both significantly reduce the computation time and to provide metric scale which is crucial for lidar depth loss. We use submapping to scale the system to large-scale environments captured over long trajectories. We demonstrate the reconstruction system with data from a multi-camera, lidar sensor suite onboard a legged robot, hand-held while scanning building scenes for 600 metres, and onboard an aerial robot surveying a multi-storey mock disaster site-building. Website: https://ori-drs.github.io/projects/silvr/
## Links
https://ieeexplore.ieee.org/abstract/document/10611278

## Status: Abstract



# Notes

### Summary


## Related Papers
